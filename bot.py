...
async def gen(ctx, tier=None, service=None):
    ...
    path = f"{ACCOUNTS_DIR}/free/{service}.txt"
    ...
    pending[code] = {"account": acc, "user": ctx.author.id, "service": service, "tier":"free"}
...
async def add(ctx, tier=None, service=None):
    ...
    if not service:
        return await ctx.send("❌ You must specify a service")
    ...
    path_dir = f"{ACCOUNTS_DIR}/{tier}"
    os.makedirs(path_dir, exist_ok=True)
    path_file = f"{path_dir}/{service}.txt"
    ...
    save_json(path_file, existing+new_accounts)

    await ctx.send(f"✅ Added {len(new_accounts)} accounts to **{tier}/{service}**")
    send_log(f"{ctx.author} added {len(new_accounts)} accounts to {tier}/{service}")
