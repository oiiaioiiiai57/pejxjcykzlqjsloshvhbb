...
HF_MODEL = "EleutherAI/gpt-neo-125M"  # modèle gratuit Hugging Face
...
async def ai(ctx, *, prompt=None):
    if not prompt:
        return await ctx.send("❌ Usage: !ai <question ou texte>")
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if isinstance(data, list) and "generated_text" in data[0]:
                ...
                await ctx.send("❌ Réponse invalide du modèle")
        else:
            await ctx.send(f"❌ Model busy or rate limited ({res.status_code})")
