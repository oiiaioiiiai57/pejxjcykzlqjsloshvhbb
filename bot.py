...
_gc=None; _ro=None
# Mapping ticket_id → channel_id pour bridge Discord→Site (reconstruit au démarrage)
...
def is_mod(m):    return any(r.id in MODERATOR_ROLES for r in m.roles)
def is_helper(m): return any(r.id==HELPER_ROLE for r in m.roles)
def has_addv(m):  return any(r.id==ADDV_ROLE for r in m.roles)
...
def next_vouch_tier(cur):
    for t,r in VOUCH_TIERS:
        if cur<t: return t,r
    return None
        ...
        if vouches>=threshold:
            ...
            for rid in role_ids:
                ...
                if role and role not in member.roles:
                    await member.add_roles(role,reason=f"Reached {threshold} vouches"); newly.append(rid)
            if newly:
                mentions=" ".join(f"<@&{r}>" for r in newly)
                log=embed_log("🎉  Promotion",f"{member.mention} → {mentions} avec **{vouches} vouches**!")
                log.color=C_PREMIUM; await send_log(guild,log)

def record_gen(uid_str, tier):
    """Save +1 gen for user, both total and per-tier."""
...
async def on_ready():
    ...
    # Reconstruire le mapping channel→ticket depuis GitHub (survit aux redémarrages)
    try:
...
async def on_message(message: discord.Message):
    ...
    # FIX: fallback GitHub si pas en RAM (après redémarrage ou ticket créé avant /redeem)
    if not ticket_id:
        try:
            import requests as _req
            r = _req.get(f"{BACKEND_URL}/internal/tickets_map",
    ...
    # Bridge le message vers le backend
    import aiohttp
    try:
...
async def gen(interaction:discord.Interaction,tier:app_commands.Choice[str],service:str):
    ...
    # ping staff but NOT the member
    await ticket.send(f"<@&{STAFF_ROLE}>",embed=te)
...
async def redeem(interaction:discord.Interaction,code:str):
    ...
    # Ne supprimer le channel que si ce n'est PAS un web ticket
    # (pour les web tickets, le bridge doit rester actif)
    if not web_ticket:
        await asyncio.sleep(5)
        if interaction.channel: await interaction.channel.delete(reason="Ticket redeemed")
    else:
        # Pour web ticket: envoyer un message de confirmation dans le channel
...
async def close(interaction:discord.Interaction):
    ...
    # Chercher si c'est un web ticket
    ticket_id = None
    for tid, t in list(_WEB_TICKETS_BY_CHANNEL.items()):
    ...
    # Notifier le site si web ticket
    if ticket_id:
        import aiohttp
        try:
            async with aiohttp.ClientSession() as sess:
                await sess.post(
                    f"{BACKEND_URL}/internal/ticket/{ticket_id}/close",
                    headers={"X-Bot-Secret": BOT_SECRET},
                )
        except Exception as e:
...
async def promote(interaction:discord.Interaction,member:discord.Member=None):
    await interaction.response.defer()
    target=member or interaction.user; vouches=get_vouches(target.id); next_t=next_vouch_tier(vouches)
    ...
    lines=[f"{'✅' if vouches>=t else '🔒'} **{t} vouches** → {' · '.join(f'<@&{r}>' for r in rs)}" for t,rs in VOUCH_TIERS]
...
async def profile(interaction:discord.Interaction,member:discord.Member=None):
    ...
    # Total gens = only numeric uid keys
...
async def leaderboard(interaction:discord.Interaction):
    await interaction.response.defer()
    stats=load_json(STATS_FILE)
    # Only keep pure numeric Discord user IDs with int values
