...
def count_active_cooldowns(cd, tier):
    now = int(time.time())
    period = {"free": 3600, "premium": 3600, "paid": 3600}[tier]
    return len([t for t in cd.get(tier, []) if now - t < period])
...
async def profile(ctx, member: discord.Member = None):
    ...
    embed.add_field(name="Free", value=f"{count_active_cooldowns(cd,'free')}/1")
    embed.add_field(name="Premium", value=f"{count_active_cooldowns(cd,'premium')}/3")
    embed.add_field(name="Paid", value=f"{count_active_cooldowns(cd,'paid')}/10")
