...
async def add(ctx, service):
    if ctx.author.id not in AUTHORIZED_IDS: return await ctx.send("Owner only")
    if not ctx.message.attachments: return await ctx.send("Attach a txt file")
    attachment = ctx.message.attachments[0]
    new_accounts = (await attachment.read()).decode().splitlines()
    path = f"accounts/{service}.txt"
    try:
        file = repo.get_contents(path)
...
async def send(ctx, amount:int, service, member:discord.Member):
    if ctx.author.id not in AUTHORIZED_IDS: return
    accounts, sha = load_accounts(service)
    if len(accounts)<amount: return await ctx.send("Not enough stock")
...
async def leaderboard(ctx):
    ...
    for i,(uid,count) in enumerate(sorted_stats[:10],start=1):
        ...
        embed.add_field(name=f"{i+1}. {member.name if member else uid}", value=f"{count} accounts")
    await ctx.send(embed=embed)

# ---------------- ASK (IA) ----------------
@bot.command()
async def ask(ctx, *, question=None):
    if not question: return await ctx.send("❌ Usage: !ask <question>")
    try:
