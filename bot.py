...
async def send_log(guild, embed):
    ...
    if channel:
        ...
        print(f"❌ LOG_CHANNEL {LOG_CHANNEL} not found in guild {guild.name}")
...
async def gen(ctx, tier=None, service=None):
    ...
    await send_log(ctx.guild, discord.Embed(
        title="🎟 Ticket Created",
        description=f"{ctx.author} generated **{service} ({tier})**",
        color=0x57F287
    ))
...
async def redeem(ctx, code=None):
    ...
    # Log
    await send_log(ctx.guild, discord.Embed(
        title="📦 Account Redeemed",
        description=f"{ctx.author} redeemed **{account}** for {user.mention}",
        color=0x57F287
    ))

# ------------------ STOCK ------------------
async def send_stock(ctx, tier):
    msg = ""
    try:
        ...
        msg = "No stock"
    colors = {"free":0x57F287,"premium":0x9B59B6,"paid":0xF1C40F}
    titles = {"free":"🟢 Free Stock","premium":"🟣 Premium Stock","paid":"🟡 Paid Stock"}
    embed = discord.Embed(title=titles[tier], description=msg, color=colors[tier])
    await ctx.send(embed=embed)

@bot.command()
async def stock(ctx): await send_stock(ctx,"free"); await send_stock(ctx,"premium"); await send_stock(ctx,"paid")
@bot.command(name="stock-free") async def stock_free(ctx): await send_stock(ctx,"free")
@bot.command(name="stock-premium") async def stock_premium(ctx): await send_stock(ctx,"premium")
@bot.command(name="stock-paid") async def stock_paid(ctx): await send_stock(ctx,"paid")
