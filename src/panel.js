...
// Owner du serveur toujours autorisé
// Admin autorisé seulement si staffRoleId est déjà configuré
function canUsePanel(member, cfg) {
  if (member.id === member.guild.ownerId) return true;
  // Si staffRoleId est configuré, les admins peuvent aussi utiliser le panel
    ...
    .addFields(
      {
        ...
        value: [
          ...
          `🔵 Booster : ${f(cfg.boosterChannel)}`,
          `🔴 Extrême : ${f(cfg.extremeChannel)}`,
        ].join("\n"),
        inline: true,
      },
      {
      ...
      },
      {
        ...
        value: [
          ...
          `🔵 Booster : ${rArr(cfg.tierRoles?.booster)}`,
          `🔴 Extrême : ${rArr(cfg.tierRoles?.extreme)}`,
        ].join("\n"),
        inline: false,
      },
      {
...
function buildPanelButtons() {
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId("panel_channels_gen").setLabel("📺 Salons Gen").setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId("panel_channels_other").setLabel("🗂️ Autres Salons").setStyle(ButtonStyle.Primary),
...
// ── MODALS ────────────────────────────────────────────────────
function channelsGenModal(cfg) {
  return new ModalBuilder()
    .setCustomId("panel_modal_channels_gen")
    .setTitle("📺 Salons de génération")
    .addComponents(
      new ActionRowBuilder().addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId("boosterChannel").setLabel("Salon BOOSTER (ID)").setStyle(TextInputStyle.Short)
          .setValue(cfg.boosterChannel || "").setRequired(false).setPlaceholder("ex: 1234567890123456789"),
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId("extremeChannel").setLabel("Salon EXTRÊME (ID)").setStyle(TextInputStyle.Short)
          .setValue(cfg.extremeChannel || "").setRequired(false).setPlaceholder("ex: 1234567890123456789"),
      ),
    );
}

function channelsOtherModal(cfg) {
  return new ModalBuilder()
    .setCustomId("panel_modal_channels_other")
    .setTitle("🗂️ Autres salons")
    .addComponents(
      new ActionRowBuilder().addComponents(
...
function tierRolesModal(cfg) {
  return new ModalBuilder()
    ...
    .addComponents(
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId("tierBooster").setLabel("Rôles BOOSTER (IDs séparés par des virgules)").setStyle(TextInputStyle.Short)
          .setValue(cfg.tierRoles?.booster?.join(",") || "").setRequired(false),
      ),
      new ActionRowBuilder().addComponents(
        new TextInputBuilder().setCustomId("tierExtreme").setLabel("Rôles EXTRÊME (IDs séparés par des virgules)").setStyle(TextInputStyle.Short)
          .setValue(cfg.tierRoles?.extreme?.join(",") || "").setRequired(false),
      ),
    );
}
...
export async function handlePanel(interaction) {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === "panel") {
    const cfg0 = GUILDS[guildId] || null;
    if (!canUsePanel(interaction.member, cfg0)) {
      return interaction.reply({ content: "❌ Seul le **propriétaire du serveur** peut utiliser `/panel` (ou le staff une fois configuré).", ephemeral: true });
    }
  ...
  if (interaction.isButton()) {
    if (!canUsePanel(interaction.member, GUILDS[guildId])) {
    ...
    if (interaction.customId === "panel_channels_gen")   return interaction.showModal(channelsGenModal(cfg));
    if (interaction.customId === "panel_channels_other") return interaction.showModal(channelsOtherModal(cfg));
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (interaction.customId === "panel_modal_tier_roles") {
      ...
      if (bo.length) cfg.tierRoles.booster = bo;
      if (ex.length) cfg.tierRoles.extreme = ex;
    }
