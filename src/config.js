...
export async function loadGuildConfig() {
  try {
    ...
    // Si la config contient les anciens IDs des 2 serveurs, on la réécrit
    const hasOldServers = saved && (saved["1479080681572274320"] || saved["1479133088524009514"]);
    if (saved && Object.keys(saved).length > 0 && !hasOldServers) {
      Object.keys(GUILDS).forEach(k => delete GUILDS[k]);
      Object.assign(GUILDS, saved);
      console.log(`✅ Guild config loaded (${Object.keys(GUILDS).length} guilds)`);
    } else {
      // Écrire la nouvelle config par défaut (1 seul serveur)
      await writeJson(CONFIG_FILE, DEFAULT_GUILDS);
      Object.keys(GUILDS).forEach(k => delete GUILDS[k]);
      Object.assign(GUILDS, DEFAULT_GUILDS);
      console.log("✅ Fresh guild config written to GitHub");
    }
  } catch (e) {
