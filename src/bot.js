    ...
    if (fileExt === ".txt") {
      ...
      for (const entry of dir.files) {
        if (entry.path.endsWith(".txt") && entry.type === "File") {
          ...
          // Store the whole file content as one account entry
          const fileText = content.toString("utf8").trim();
          if (fileText) lines.push(fileText);
        }
      }
  ...
  if (name === "send") {
    ...
    for (const t of TIERS) {
      ...
      if (stock.length>=amount) {
        ...
        // Send as .txt file
        const { AttachmentBuilder: ABSend } = await import("discord.js");
        const fileContent = accs.join("\n");
        const sendFile = new ABSend(Buffer.from(fileContent, "utf8"), { name: `${service}.txt` });
        const sendEmbed = new EmbedBuilder()
          .setTitle(`📦 ${service} (${TIER_META[t].label})`)
          .setDescription(`**${accs.length}** accounts attached below.\n**Do not share this!**`)
          .setColor(TIER_META[t].color).setTimestamp();
        await target.send({embeds:[sendEmbed], files:[sendFile]}).catch(()=>{});
