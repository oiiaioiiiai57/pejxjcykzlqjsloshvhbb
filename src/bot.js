...
async function checkAndPromote(guild, member, vouches) {
  ...
  for (const {threshold,roles} of cfg.vouchTiers) {
    if (vouches>=threshold) {
      const newly=[];
      for (const rid of roles) { if (!member.roles.cache.has(rid)) { await member.roles.add(rid).catch(console.error); newly.push(rid); } }
      if (newly.length) {
...
const commands = [
  ...
  new SlashCommandBuilder().setName("panel").setDescription("[Admin] Configurer le bot"),
...
client.on(Events.InteractionCreate, async interaction => {
  // Panel first (buttons + modals + slash)
  if (
    ...
    try { await handlePanel(interaction); }
    catch(e) {
      console.error("Panel error:", e);
      const msg = { content:`❌ Erreur panel : ${e.message}`, ephemeral:true };
      try {
...
async function handleCommand(interaction, name) {
  ...
  if (name === "gen") {
    ...
    if (!cfg) return interaction.followUp({embeds:[err("Erreur","Serveur non configuré. Utilise /panel.")]});
  ...
  if (name === "help") {
    const embed=new EmbedBuilder().setTitle("📜  Commands — Gen Bot").setColor(C.info)
      .addFields(
        ...
        {name:"⚙️  Admin",   value:"`/panel`",inline:false},
