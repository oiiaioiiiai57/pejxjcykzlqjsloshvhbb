...
async function endGiveaway(messageId) {
  ...
  try {
    ...
    if (cfg) {
      const l = log("🎁 Giveaway terminé",`Winner: ${winner} • **${gw.service}** (${gw.tier})`);
...
async function handleCommand(interaction, name) {
  ...
  if (name === "gen") {
    ...
    if (!cfg) return interaction.followUp({embeds:[err("Erreur","Server not configured.")]});
  ...
  if (name === "redeem") {
    ...
    if (!webTicketId) {
      ...
      await interaction.channel.send({embeds:[
        new EmbedBuilder().setTitle("✅ Account Validated").setDescription(`Transmis via le ticket web.`).setColor(C.success)
      ]});
    }
    return;
  }
...
const botHttpServer = http.createServer(async (req, res) => {
  ...
  req.on("end", async () => {
    try {
      ...
      // Créer le salon — hérite des perms de la catégorie, pas de permissionOverwrites
      const ticketCh = await guild.channels.create({
        name,
