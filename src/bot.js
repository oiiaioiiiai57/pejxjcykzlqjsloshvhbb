...
client.on(Events.PresenceUpdate, async (oldPresence, newPresence) => {
  try {
    ...
    // Check if bio/activities contain the link
    const activities = newPresence.activities || [];
    const hasBioLink = activities.some(a =>
      a.state?.includes(cfg.bioLink) ||
      a.details?.includes(cfg.bioLink) ||
      a.name?.includes(cfg.bioLink)
    );

    // Also check Discord profile bio via user.fetch
    const user = await newPresence.user?.fetch(true).catch(()=>null);
    const bio  = user?.bio || "";
    const hasLink = hasBioLink || bio.includes(cfg.bioLink);
...
// Also check on member join
client.on(Events.GuildMemberAdd, async (member) => {
  const cfg = getCfg(member.guild.id);
  if (!cfg?.notVerifiedRole) return;
  // Give not-verified role on join (server 2 only)
  if (member.guild.id !== "1479080681572274320") return;
...
async function handleCommand(interaction, name) {
  ...
  if (name === "stock") {
    ...
    const serverTiers = guild.id === "1479080681572274320" ? ["free","premium","paid"] : TIERS;
