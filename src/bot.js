...
async function buildAccountDM(account, service, tier) {
  ...
  if (account.startsWith("FILE:")) {
    // Full file content stored as base64
    const content = Buffer.from(account.slice(5), "base64");
    const file = new AttachmentBuilder(content, {name:`${service}.txt`});
    embed.setDescription("Your account file is attached below.\n**Do not share this with anyone!**");
    return { embeds:[embed], files:[file] };
  } else {
    // Single line account — send as .txt file
    const file = new AttachmentBuilder(Buffer.from(account, "utf8"), {name:`${service}.txt`});
    embed.setDescription("Your account file is attached below.\n**Do not share this with anyone!**");
    return { embeds:[embed], files:[file] };
  }
}
    ...
    if (fileExt === ".txt") {
      ...
      for (const entry of dir.files) {
        if (entry.path.endsWith(".txt") && entry.type === "File") {
          ...
          if (fileText) {
            // Prefix with FILE: + base64 so we know it's a full file on delivery
            const encoded = "FILE:" + Buffer.from(fileText).toString("base64");
            lines.push(encoded);
          }
        }
      }
  ...
  if (name === "send") {
    ...
    for (const t of TIERS) {
      ...
      if (stock.length>=amount) {
        ...
        // Send each account as file
        for (const acc of accs) {
          const sendPayload = await buildAccountDM(acc, service, t);
