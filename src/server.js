...
function getUserTierForGuild(roleIds, guildCfg) {
  ...
  if (guildCfg.tierRoles.paid.some(r => set.has(r)))    return "paid";
  if (guildCfg.tierRoles.premium.some(r => set.has(r))) return "premium";
  if (guildCfg.tierRoles.free.some(r => set.has(r)))    return "free";
  return null;
}
