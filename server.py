...
Railway env vars needed:
  ...
  DISCORD_REDIRECT_URI, BOT_SECRET (secret partagé bot↔server)
"""
...
SESSIONS_FILE = "web_sessions.json"   # sessions persistantes sur GitHub
GENLOG_FILE   = "web_genlog.json"      # historique des gens par user
TICKETS_FILE  = "web_tickets.json"    # tickets persistants
...
# ── MÉMOIRE RUNTIME ──────────────────────────────────────────
COOLDOWNS   = {}   # { user_id: { tier: [timestamps] } }
WEB_TICKETS = {}   # { ticket_id: { ... } }  — chargé depuis GitHub au démarrage

def _load_tickets():
    """Charge les tickets depuis GitHub dans WEB_TICKETS au démarrage."""
    global WEB_TICKETS
    try:
...
def get_session(token):
    """Vérifie d'abord la RAM puis GitHub."""
    # RAM cache
...
_session_cache = {}  # RAM cache pour perf
_genlog_cache = {}   # RAM cache genlog pour perf
...
def discord_create_ticket_channel(user_id, username, service, tier, code, ticket_id):
    """Crée le salon ticket Discord, ping staff (PAS le membre), retourne channel_id."""
    ...
    overwrites = [
        ...
        {"id": str(user_id), "type": 1, "allow": "0"},                                  # member: no view (web only)
        {"id": str(STAFF_ROLE), "type": 0, "allow": "1024"},                            # staff: view
    ]
    ...
    # Ping staff uniquement (pas le membre)
...
def web_gen():
    ...
    # Charger les tickets si pas encore fait
    if not WEB_TICKETS: _load_tickets()

    # Stocker le ticket web
    ...
    # Persister le ticket sur GitHub
    _save_ticket(ticket_id)

    # Log historique gen de l'user (sans le compte, révélé après redeem)
    ...
    genlog[uid_key].insert(0, {
        ...
        "account": None,  # rempli après /redeem
    })
...
def _get_ticket(ticket_id):
    """Récupère un ticket depuis la mémoire ou GitHub."""
    if ticket_id in WEB_TICKETS:
        return WEB_TICKETS[ticket_id]
    # Fallback GitHub (après redémarrage Railway)
    all_t = load_json(TICKETS_FILE)
...
def close_ticket_web(ticket_id):
    """Fermeture depuis le site (staff only)."""
    token   = request.headers.get("X-Session-Token")
    session = get_session(token)
    if not session: return jsonify({"error":"Not logged in"}),401
...
def internal_redeem(ticket_id):
    ...
    # Mettre à jour le genlog avec le compte maintenant qu'il est validé
    user_id = t["user_id"]
    try:
        genlog  = load_json(GENLOG_FILE)
        uid_key = str(user_id)
...
def internal_message(ticket_id):
    """Message Discord → site (bridgé depuis le bot)."""
    if request.headers.get("X-Bot-Secret") != BOT_SECRET:
        return jsonify({"error":"Forbidden"}),403
    t = WEB_TICKETS.get(ticket_id)
...
def tickets_map():
    """Retourne le mapping ticket_id → discord_channel_id pour le bot."""
    if request.headers.get("X-Bot-Secret") != BOT_SECRET:
        return jsonify({"error":"Forbidden"}),403
    result = {}
    # S'assurer que les tickets sont chargés
    if not WEB_TICKETS:
        _load_tickets()
