...
def _load_tickets():
    """Charge les tickets depuis GitHub dans WEB_TICKETS au démarrage."""    global WEB_TICKETS
    try:
...
def _save_ticket(ticket_id):
    """Sauvegarde un ticket modifié dans GitHub."""    try:
...
def _get_ticket(ticket_id):
    """Récupère un ticket depuis la mémoire ou GitHub."""    if ticket_id in WEB_TICKETS:
...
def get_profile():
    ...
    genlog = load_json(GENLOG_FILE).get(uid, [])
